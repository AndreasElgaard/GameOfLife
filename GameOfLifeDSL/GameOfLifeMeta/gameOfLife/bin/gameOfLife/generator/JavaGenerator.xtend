...
class JavaGenerator {
	...
	def static String getEvolutionRules(EvolutionRule r) {
		if (r.action == Action::LIVE) {
			return '''
				if (gameBoard[i][j] && (surrounding «getConditionElement(r.conditionElement)»«r.amount»)){
					survivingCells.add(new Point(i-1,j-1));
				}
				'''
		} else if (r.action == Action::BECOMEALIVE) {
			return '''
				if ((!gameBoard[i][j]) && (surrounding «getConditionElement(r.conditionElement)»«r.amount»)){
					survivingCells.add(new Point(i-1,j-1));
				}
			'''
		} else if (r.action == Action::DIE) {
			return '''
				if (gameBoard[i][j] && (surrounding «getConditionElement(r.conditionElement)»«r.amount»)){
					continue;
				}
